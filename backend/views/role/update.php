    ...
    <form action="" method="post" class="layui-form layui-form-pane">
        ...
        <div class="layui-form-item layui-form-text">
            ...
            <div class="layui-input-block">
                <textarea placeholder="请输入内容" id="description" name="description" class="layui-textarea" style="min-height: 50px"><?= $role['description'] ?></textarea>
            </div>
        </div>
        <div class="layui-form-item layui-form-text">
                        ...
                        <td>
                            <div class="layui-input-block">
                                <?php foreach ($list['route'] as $item): ?>
                                    ...
                                    <?= in_array($item['id'], json_decode($role['permission'])) ? 'checked' : '' ?>>
                                <?php endforeach; ?>
                            </div>
                        </td>
                    </tr>
...
<script>
    layui.use(['form','layer'], function(){
        ...
        form.on('submit(update)', function(data){
            layer.load(3);
            $.post('<?= Url::to(['role/update']) ?>', data.field, function (res) {
                layer.closeAll();
                if (res.status === 200) {
